...
public class DeleteUserPost extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet DeleteUserPost</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet DeleteUserPost at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (userId == null || userPostId == null || userId.trim().equals("") || userPostId.trim().equals("")) {
            response.sendRedirect("ErrorPage");
        } else {
            if (!userId.equals(userAuthorization.getUser().getUserId() + "")) {
                response.sendRedirect("ErrorPage");
            } else {
                UserPostService userPostService = new UserPostService();
                UserPost userPost = UserPost.builder().userPostId(0L).build();
                try {
                    userPost = userPostService.getById(userPostId);
                } catch (Exception e) {
                }
                if (!userPostId.equals(userPost.getUserPostId() + "")) {
                    response.sendRedirect("ErrorPage");
                } else {
                    try {
                        userPostService.softDeleteUserPostById(userPost.getUserPostId()+"");
                    } catch (Exception e) {
                    }
                    response.sendRedirect("ForumPage");
                }
            }
        }
    }
